...
```
http://localhost:3008/api/{endpoint}
```
...
```javascript
// Requisição
GET http://localhost:3008/api/clientes?page=1&limit=10&search=João
...
```javascript
// Requisição
POST http://localhost:3008/api/clientes
{
  "nome": "Maria Santos",
...
```bash
curl http://localhost:3008/api/clientes
```
